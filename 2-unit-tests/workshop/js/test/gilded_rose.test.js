...
describe("Names", function () {
  ...
  it("should get better", function () {
    ...
    const Backstage2 = new Shop([
      new Item("Backstage passes to a TAFKAL80ETC concert", 10, 20),
    ]);
    const Backstage3 = new Shop([
      new Item("Backstage passes to a TAFKAL80ETC concert", 5, 20),
    ]);
    const BackstageOld = new Shop([
...
describe("Product quality", function () {
  it("should be under 50", function () {
    const gildedRose = new Shop([new Item("bottle", 0, 20)]);
    const items = gildedRose.updateQuality();
    expect(items[0].quality).toBeLessThan(50);
  });

  it("should be positive", function () {
    const gildedRose = new Shop([new Item("candy", 0, 12)]);
    const items = gildedRose.updateQuality();
    expect(items[0].quality).toBeGreaterThanOrEqual(0);
  });

  it("should be an integer", function () {
    const gildedRose = new Shop([new Item("candy", 0, 25)]);
    const items = gildedRose.updateQuality();
